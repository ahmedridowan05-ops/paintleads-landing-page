...
export const IdealClientsSection: React.FC<IdealClientsSectionProps> = ({ onOpenCheckModal }) => {
  ...
  return (
    <section id="ideal-clients" className="py-10 sm:py-16 md:py-28 bg-[#180c0e] relative overflow-hidden border-t border-[#3b2126]">
      ...
      <div className="max-w-5xl mx-auto px-5 sm:px-6 lg:px-8 space-y-8 sm:space-y-12 md:space-y-16 relative z-10">
        ...
        >
          ...
          <p className="text-[#C2B29F] text-[14px] sm:text-[16px] md:text-lg max-w-full xs:max-w-sm sm:max-w-xl md:max-w-2xl mx-auto leading-[1.55]">
          ...
          {cards.map((card) => {
            ...
            return (
              <motion.div
                ...
                key={card.id}
                className="bg-gradient-to-br from-[#221215] to-[#1B0D10] p-4 sm:p-6 md:p-8 rounded-2xl md:rounded-3xl border border-[#3b2126]/60 hover:border-[#E0B669]/40 transition-all duration-300 group flex flex-row gap-3 sm:gap-5 items-start shadow-sm hover:shadow-xl hover:shadow-[#E0B669]/5"
              >
                ...
                <div className="space-y-1 sm:space-y-3">
                  <h3 className="text-[15.5px] sm:text-[20px] md:text-[22px] font-normal font-serif-display text-[#F6F2EA] leading-tight group-hover:text-[#E0B669] transition-colors">
                    {card.title}
                  </h3>
        ...
        <motion.div 
          ...
          className="text-center pt-4 sm:pt-4 md:pt-8 w-full"
        >
          <button
